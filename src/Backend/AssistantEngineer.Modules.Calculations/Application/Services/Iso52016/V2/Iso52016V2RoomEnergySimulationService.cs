...
public sealed class Iso52016V2RoomEnergySimulationService : IIso52016V2RoomEnergySimulationService
{
    ...
    private readonly IIso52016V2ReducedRoomModelBuilder _reducedRoomModelBuilder;
    private readonly IIso52016V2HourlySolver _hourlySolver;

    public Iso52016V2RoomEnergySimulationService(
        ...
        IIso52016V2ReducedRoomModelBuilder reducedRoomModelBuilder,
        IIso52016V2HourlySolver hourlySolver)
    {
    ...
    public Result<Iso52016V2RoomEnergySimulationResult> Simulate(
        Iso52016RoomEnergySimulationRequest request)
    {
        var validation = Validate(request);

        if (validation.IsFailure)
            return Result<Iso52016V2RoomEnergySimulationResult>.Failure(validation);
        ...
        if (solarGainResult.IsFailure)
            return Result<Iso52016V2RoomEnergySimulationResult>.Failure(solarGainResult);
        ...
        if (internalGainResult.IsFailure)
            return Result<Iso52016V2RoomEnergySimulationResult>.Failure(internalGainResult);
        ...
        if (hourlyInputResult.IsFailure)
            return Result<Iso52016V2RoomEnergySimulationResult>.Failure(hourlyInputResult);
        ...
        if (matrixRequestResult.IsFailure)
            return Result<Iso52016V2RoomEnergySimulationResult>.Failure(matrixRequestResult);

        var matrixResult = _hourlySolver.Solve(matrixRequestResult.Value);

        if (matrixResult.IsFailure)
            return Result<Iso52016V2RoomEnergySimulationResult>.Failure(matrixResult);

        return Result<Iso52016V2RoomEnergySimulationResult>.Success(
    ...
    {
        if (request is null)
            return Result.Validation("ISO 52016 V2 room energy simulation request is required.");
