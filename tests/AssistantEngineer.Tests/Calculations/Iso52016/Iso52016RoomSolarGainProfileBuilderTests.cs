...
public class Iso52016RoomSolarGainProfileBuilderTests
{
    ...
    [Fact]
    public void Build_UsesSurfaceIrradianceForWindowOrientation()
    {
        var context = CreateContextWithSouthAndNorthSurfaces();
        ...
        var firstHour = result.Value.GetHour(0);
        var south = firstHour.WindowGains.Single(window => window.WindowCode == "South");
        var north = firstHour.WindowGains.Single(window => window.WindowCode == "North");

        Assert.Equal(600.0, south.TotalSolarGainW, precision: 6);
        Assert.Equal(60.0, north.TotalSolarGainW, precision: 6);
        Assert.Equal(660.0, firstHour.TotalSolarGainW, precision: 6);
    }
    ...
    private static Iso52016WeatherSolarContext CreateContextWithSouthAndNorthSurfaces()
    {
        var hours = Enumerable
            .Range(0, 24)
            .Select(hour => new Iso52016HourlyWeatherSolarRecord(
                ...
                [
                    new Iso52016SurfaceWeatherSolarRecord(
                        ...
                        TotalIrradianceWm2: 500),
                    new Iso52016SurfaceWeatherSolarRecord(
                        SurfaceCode: WeatherSolarSurfaceCodes.North,
                        Orientation: WeatherSolarSurface.North.Orientation,
                        IncidenceAngleDegrees: 120,
                        BeamIrradianceWm2: 0,
                        DiffuseSkyIrradianceWm2: 50,
                        GroundReflectedIrradianceWm2: 0,
                        TotalIrradianceWm2: 50)
                ]))
            .ToArray();
