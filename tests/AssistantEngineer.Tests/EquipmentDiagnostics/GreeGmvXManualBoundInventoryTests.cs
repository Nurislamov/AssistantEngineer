...
public sealed class GreeGmvXManualBoundInventoryTests
{
    ...
    public void GmvXInventoryClassificationHasNoConflictOrUnclassifiedRows()
    {
        ...
        Assert.Equal(70, classifiedEntries.Count(entry => entry.RepairClass == "AlreadyRepaired"));
        Assert.Equal(95, classifiedEntries.Count(entry => entry.RepairClass == "DetailedProcedureAvailable"));
        ...
        Assert.All(new[] { "A0", "A2", "A3", "A4", "AJ", "db", "UC", "b1", "bJ", "bn", "E1", "Ed", "F5", "F9", "FH", "FU" }, code =>
