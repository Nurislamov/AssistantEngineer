...
public sealed class GreeXFlexInventory13Tests
{
    ...
    public void RuntimeCountsAndXFlexFoldersRemainUnchanged()
    {
        ...
        {
            Assert.False(Directory.Exists(Path.Combine(GreeRuntimeDirectory, folder)), $"Inventory stage must not create runtime folder: {folder}");
        }
        ...
        Assert.DoesNotContain(packageNames, name => name.Contains("gmv-x", StringComparison.OrdinalIgnoreCase));
