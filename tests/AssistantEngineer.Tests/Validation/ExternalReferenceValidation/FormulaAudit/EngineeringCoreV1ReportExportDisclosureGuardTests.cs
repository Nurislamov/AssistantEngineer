...
public class EngineeringCoreV1ReportExportDisclosureGuardTests
{
    ...
    public void ExportDisclosurePolicyDocumentsAnnual8760RulesAndNonClaims()
    {
        ...
        Assert.Contains("no exact EnergyPlus numerical equivalence", content, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("no exact StandardReference numerical equivalence", content, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("no ASHRAE 140 / BESTEST-style validation anchor coverage", content, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("no full ISO 52016 node/matrix solver equivalence", content, StringComparison.OrdinalIgnoreCase);
    ...
    public void ReportSnapshotsKeepExportCriticalNonClaimsVisible()
    {
        foreach (var snapshotPath in SnapshotPaths)
        {
            ...
            Assert.Contains(nonClaims, claim => claim.Contains("No exact StandardReference", StringComparison.OrdinalIgnoreCase));
    ...
    public void GeneratorScriptReadsSnapshotsAndWritesChecklist()
    {
        ...
        Assert.Contains("No exact EnergyPlus numerical equivalence claim.", tool, StringComparison.Ordinal);
    }
