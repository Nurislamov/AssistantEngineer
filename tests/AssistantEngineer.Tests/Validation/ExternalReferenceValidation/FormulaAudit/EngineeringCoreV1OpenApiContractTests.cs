...
public class EngineeringCoreV1OpenApiContractTests
{
    ...
    public void OpenApiFragmentKeepsAnnual8760FlagsAndNonClaimsVisible()
    {
        ...
        Assert.Contains("No exact EnergyPlus numerical equivalence claim.", content, StringComparison.Ordinal);
        Assert.Contains("No exact StandardReference numerical equivalence claim.", content, StringComparison.Ordinal);
        Assert.Contains("No ASHRAE 140 / BESTEST-style validation anchor coverage claim.", content, StringComparison.Ordinal);
        Assert.Contains("No full ISO 52016 node/matrix solver equivalence claim.", content, StringComparison.Ordinal);
    }
