...
| --- | --- | ---: | --- | --- |
| Backend integration tooling | `src/Backend/AssistantEngineer.Infrastructure/Integrations/Benchmarks/EnergyPlusModelExporter.cs` | 562 | Export lane remains integration-sensitive; drift risk is mostly in deterministic IDF text assembly. | P4 partial done: exporter now delegates to section builders (`Header`, `Schedule`, `Geometry`, `Window`, `InternalGains`, `VentilationInfiltration`, `IdealLoads`, `Output`) with deterministic/order guards; keep this lane benchmark/export only (no exact EnergyPlus numerical equivalence claim). |
| Reporting assembly | `src/Backend/AssistantEngineer.Modules.Reporting/Application/Services/EngineeringReportSectionBuilder.cs` | 124 | Section behavior remains contract-sensitive, but orchestration pressure is reduced by focused builders. | P4 partial done: section families extracted into focused builders (`ExecutiveSummary`, `InputSummary`, `WeatherSolar`, `LoadResults`, `SystemEnergy`, `GovernanceMetadata`) with ordering/metadata/missing-data diagnostics guards. |
| ISO52016 hourly core | `src/Backend/AssistantEngineer.Modules.Calculations/Application/Services/Iso52016/Iso52016HourlyHeatBalanceCalculator.cs` | 515 | High-risk core numeric flow remains sensitive; numeric kernel must stay operation-order stable. | P4 partial done: extracted validation/request-normalization/diagnostics/result-assembly helpers; continue staged extraction around numeric kernel only. |
| Topology aggregation | `src/Backend/AssistantEngineer.Modules.Calculations/Application/Services/Topology/ThermalZoneBoundaryCalculator.cs` | 219 | Boundary behavior is still high-impact, but facade complexity is reduced and helper seams are explicit. | P4 partial done: boundary classification, adjacent resolution, diagnostics formatting, aggregation and result assembly extracted; continue targeted regression coverage. |
| Ventilation load engine | `src/Backend/AssistantEngineer.Modules.Calculations/Application/Services/Ventilation/VentilationAndInfiltrationLoadEngine.cs` | 230 | Scenario branching was previously concentrated in one class; regression safety depends on stable helper seams. | P4 partial done: extracted `VentilationInputNormalizer`, scenario evaluators, and `VentilationDiagnosticsBuilder`; keep shared numeric policy centralized. |
| Natural ventilation loads | `src/Backend/AssistantEngineer.Modules.Calculations/Application/Services/Ventilation/NaturalVentilationZoneLoadCalculator.cs` | 218 | Zone-level orchestration remains sensitive to diagnostics/order regressions during further extraction. | P4 partial done: extracted input normalization, scenario evaluation, and zone result aggregation helpers; continue targeted regression coverage. |
...
- EnergyPlus exporter lane is benchmark/export helper only and does not claim exact EnergyPlus numerical equivalence.
