...
- ExternalReferenceValidationMatrix
- ExternalReferenceValidationFeature
...
equivalence означает, что AssistantEngineer на одинаковых входных данных должен выдавать результаты, совпадающие с выбранным эталонным расчётом в пределах заданной tolerance.
...
- Public method query values are preserved as `requestedMethod`; results also expose `actualMethod` and diagnostics when the endpoint is currently using the Energy Calculation equivalence design-point pipeline for API compatibility.
...
- No feature is marked `ExternalReferenceCovered` in this pass.
...
| Code | Feature | Reason |
|---|---|---|
| LATENT.ENERGY_NEED | Latent energy need | Не входит в текущий equivalence target |
| LATENT.MOISTURE_LOAD | Moisture / latent load | Не входит в текущий equivalence target |
| SUPPLY_AIR.HUMIDIFICATION_CONDITIONS | Supply-air humidification/dehumidification conditions | Не входит в текущий equivalence target |

## Порядок реализации

### Sprint 1

Создать ExternalReferenceValidation matrix и guard-тесты.
...
`BenchmarkCompared` means a passing comparison test exists against fixed expected benchmark/reference values. It does not automatically mean `ExternalReferenceCovered`.

`ExternalReferenceCovered` requires a documented external source, fixed input, expected output, tolerance, passing comparison test, and source/version note.
...
Эти fixtures являются deterministic reference fixtures AssistantEngineer. Они не являются external reference equivalence proof и не переводят matrix features в covered.

| Fixture | Scope | Test coverage |
|---|---|---|
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/single-zone-no-solar.json` | Single-zone hourly heat balance without solar gains | Heating/cooling load, fixed transmission and ventilation coefficients, internal gains |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/single-zone-solar-south-window.json` | Single south-facing window solar gains | Beam, diffuse sky, ground-reflected and total solar gains |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/single-zone-annual-8760.json` | Compact one-zone annual aggregation | 8760-hour month bins, monthly sums, annual totals, peak loads |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/transmission-single-external-wall-winter.json` | Transmission heat transfer | Single outdoor wall winter heat loss |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/transmission-single-window-winter.json` | Transmission heat transfer | Single outdoor window winter heat loss |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/transmission-adiabatic-internal-wall.json` | Transmission heat transfer | Internal adiabatic boundary exclusion and diagnostic |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/transmission-adjacent-conditioned-same-temperature.json` | Transmission heat transfer | Adjacent conditioned zone with zero temperature difference |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/transmission-outdoor-cooling-gain.json` | Transmission heat transfer | Outdoor cooling condition heat gain sign convention |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/window-solar-single-window-no-shading.json` | Window solar gains | Single window, no shading, provided incident irradiance |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/window-solar-single-window-with-shading.json` | Window solar gains | Frame, internal shading, external shading and fixed factors |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/window-solar-night-is-zero.json` | Window solar gains | Zero incident irradiance returns zero solar gain |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/window-solar-invalid-shgc-diagnostics.json` | Window solar gains | Invalid SHGC produces diagnostics and no gain |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/window-solar-room-aggregation.json` | Window solar gains | Room-level aggregation across two windows |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/ventilation-mechanical-heating-load.json` | Ventilation and infiltration loads | Mechanical ventilation winter heating load |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/ventilation-mechanical-cooling-load.json` | Ventilation and infiltration loads | Mechanical ventilation summer cooling load |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/ventilation-with-heat-recovery.json` | Ventilation and infiltration loads | Heat recovery reduces mechanical ventilation load |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/ventilation-infiltration-by-ach.json` | Ventilation and infiltration loads | Infiltration airflow from ACH and room volume |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/ventilation-zero-airflow.json` | Ventilation and infiltration loads | Zero airflow returns zero outdoor air load |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/ventilation-invalid-heat-recovery-efficiency.json` | Ventilation and infiltration loads | Invalid heat recovery efficiency produces diagnostics |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-occupancy-sensible.json` | Internal gains | Occupancy sensible gain |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-lighting-by-area.json` | Internal gains | Lighting gain by area |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-equipment-by-area.json` | Internal gains | Equipment gain by area |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-process-with-schedule.json` | Internal gains | Process gain with schedule |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-room-aggregation.json` | Internal gains | Room internal gain aggregation |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-zero-schedule.json` | Internal gains | Zero schedule gives zero gain |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-invalid-schedule-factor.json` | Internal gains | Invalid schedule diagnostic |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/internal-gains-negative-power-density.json` | Internal gains | Negative power density diagnostic |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/room-load-heating-transmission-only.json` | Room load | Heating transmission-only total and W/m2 |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/room-load-heating-transmission-ventilation-infiltration.json` | Room load | Heating transmission, ventilation and infiltration aggregation |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/room-load-cooling-solar-internal-ventilation.json` | Room load | Cooling component separation and W/m2 |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/room-load-does-not-go-negative.json` | Room load | Negative component clamp diagnostics |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/aggregation-floor-two-rooms.json` | Load aggregation | Floor sum, area and W/m2 |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/aggregation-building-two-floors.json` | Load aggregation | Building sum from room loads on two floors |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/aggregation-thermal-zone-no-double-count.json` | Load aggregation | Thermal zone de-duplication |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/annual-constant-heating-load.json` | Annual energy balance | 8760 hourly heating W to kWh |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/annual-constant-cooling-load.json` | Annual energy balance | 8760 hourly cooling W to kWh |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/annual-monthly-aggregation-consistency.json` | Annual energy balance | Annual total equals monthly sum |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/annual-energy-use-intensity.json` | Annual energy balance | EUI from annual total and area |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/dhw-residential-simple.json` | DHW demand | People-based DHW formula |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/dhw-zero-occupancy.json` | DHW demand | Zero occupancy diagnostic |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/system-heating-efficiency.json` | System energy | Useful heating divided by efficiency |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/system-cooling-cop.json` | System energy | Useful cooling divided by COP |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/system-total-energy.json` | System energy | Total final energy |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/equipment-sizing-cooling-simple.json` | Equipment sizing | Cooling reserve from cooling safety factor |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/equipment-candidate-accepted.json` | Equipment sizing | Candidate accepted and margin |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/equipment-candidate-rejected.json` | Equipment sizing | Candidate rejected with reason |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/Fixtures/equipment-no-equipment-found.json` | Equipment sizing | Empty catalog diagnostics |

## Benchmark fixtures added

These fixtures are deterministic benchmark references for the benchmark comparison infrastructure. They are active and must pass comparison tests, but they are not external equivalence proof.

| Fixture | Scope | Status |
|---|---|---|
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/annual-constant-heating-8760.json` | Annual constant heating demand aggregation | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/annual-constant-cooling-8760.json` | Annual constant cooling demand aggregation | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/signed-component-balance-winter.json` | Signed winter transmission, ventilation and ground balance | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/signed-component-balance-summer.json` | Signed summer transmission, ventilation and ground balance with gains | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/signed-component-balance-with-infiltration-winter.json` | Signed winter ventilation and separate infiltration balance | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/signed-component-balance-with-ventilation-split-winter.json` | Signed winter mechanical/natural ventilation split with separate infiltration balance | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/solar-night-zero.json` | Window solar gain night-zero deterministic fixture | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/window-solar-gain-basic.json` | Basic deterministic window solar gain | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/window-solar-gain-with-shading.json` | Deterministic window solar gain with shading factors | BenchmarkCompared |
| `tests/AssistantEngineer.Tests/Validation/ExternalReferenceValidation/BenchmarkFixtures/Fixtures/surface-irradiance-night-zero.json` | Surface irradiance night-zero deterministic fixture | BenchmarkCompared |
...
No benchmark fixture in this section claims `ExternalReferenceCovered`.

## Fixtures still needed

| Needed fixture | Purpose |
|---|---|
| External reference single-zone annual ISO 52016 case | Real equivalence proof against an independently produced reference result |
...
The Energy Calculation equivalence true hourly path now supports signed component balance for available hourly components.
...
This update does not change external equivalence status.
...
- Not ExternalReferenceCovered.

External equivalence still requires benchmark comparison fixtures with documented source results and tolerances.
