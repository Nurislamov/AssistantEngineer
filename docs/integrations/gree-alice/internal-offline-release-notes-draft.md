...
- dev-only OAuth/provider vertical smoke slice;
...
Optional dev-only OAuth provider smoke when the isolated bridge API is running:
...
4. Use [Yandex OAuth provider pilot contract](./yandex-oauth-provider-pilot-contract.md), [pilot config example](./yandex-oauth-provider-pilot-config.example.json), and [dev smoke](./yandex-oauth-provider-dev-smoke.md) before any production pilot package.

Production Yandex release remains NOT READY.
Next implementation stage: GREE-ALICE-PILOT-2.
