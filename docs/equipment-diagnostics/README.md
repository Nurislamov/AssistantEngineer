...
## ED-24E.4 / ED-24E.5 U-Match and ERV imports

The [Gree U-Match R32 manual coverage](gree-umatch-r32-manual-coverage.md) records the 107-card import from the
confirmed U-Match R32 service-manual error-code table. The [Gree ERV B Series manual coverage](gree-erv-b-series-manual-coverage.md)
records the 2-card import for confirmed ERV diagnostic codes E6 and L0. Source PDFs remain local intake only and are
not committed.
...
The [Gree GMV6 manual import report](gree-gmv6-manual-import.md) records the single-manual source boundary, package
counts, H5 correction, model scope, and deliberately unextracted troubleshooting details.
...
## ED-15B Bot Diagnostic API

ED-15B adds one backend HTTP endpoint:
...
The controller performs only HTTP body validation and delegates to `IEquipmentDiagnosticBotFacade`. Responses may be `Answer`, `ClarificationRequired`, `ReferenceOnly`, `NotFound`, or `Unsupported`, while HTTP transport remains deterministic. Runtime catalog entries are the only source for final answers; staging candidates, manual codebook occurrences, generated previews, and local manuals remain non-runtime.
