...
runtime metadata only. The emergency repair scope reviews all 1296 runtime cards for forbidden generic wording,
normalizes 235 affected GMV6 cards to short table-only-safe answers, and fully repairs GMV6 `AJ` and `b1` from their
exact diagnostic sections. `AJ` now identifies the filter-clean prompt; `b1` contains the documented 30-second
diagnosis, three causes, and flowchart sequence.

The deterministic audit is generated by
`scripts/equipment-diagnostics/invoke-gree-manual-bound-card-audit.ps1` under the ignored
`artifacts/verification/equipment-diagnostics/` directory. Runtime counts remain unchanged, and no source PDF or other
manual binary is committed. The remaining 1294 cards are explicitly tracked as `NeedsManualReview`; this stage does not
claim a full manual-section repair. See [the ED-24SRC.1 repair report](manual-bound-card-repair.md).

## ED-24SRC.9 remaining GMV6 closure inventory

ED-24SRC.8d closes the GMV6 outdoor manual-bound scope. ED-24SRC.9 refreshes the GMV6-only closure inventory and records
the remaining non-outdoor work without changing runtime cards: indoor has 26 detailed and 34 table-only rows, status has
