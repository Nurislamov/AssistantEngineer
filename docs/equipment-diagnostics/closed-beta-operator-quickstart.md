...
5. Configure one reviewed `AllowedChatIds` or `AllowedUsernames` entry, retain the `DeniedChatIds` policy, and verify access before enabling transport.
6. Use polling mode by default; run `deleteWebhook`/`getWebhookInfo`, then smoke-test the closed beta. Use `setWebhook` only for reviewed webhook fallback.
