...
| Window Solar Gains | BenchmarkCompared | Engine tests, solar fixtures, room cooling application pipeline integration, annual-climate source/fallback diagnostics, deterministic window/surface benchmark fixtures; not ExternalReferenceCovered |
...
| ISO52016-style Multi-Zone Calculation | InternalDeterministicTested | Standard-based multi-zone deterministic fixtures and coupled hourly tests for inter-zone conductance, adjacent unconditioned boundaries, and same-use adiabatic-style boundaries; internal engineering anchor only, not external validation |
...
No function is marked ExternalReferenceCovered in this pass.
...
- ISO52016-style multi-zone calculation is available as a deterministic internal stage with explicit non-claims. It is positioned as a standard-based multi-zone calculation anchor, not as a full ISO52016 compliance or external validation claim.
...
Window solar gains are `BenchmarkCompared` for deterministic window fixtures. Surface irradiance night clamping is also `BenchmarkCompared` through deterministic benchmark fixtures. This does not claim `ExternalReferenceCovered`.
START SECTION
