...
| System Energy | InternalDeterministicTested | System energy tests, fixtures, heating/cooling system services call SystemEnergyEngine; EN15316-inspired modular chain is available as pure calculator only (no production switch yet) |
...
- Heating and cooling system services call `SystemEnergyEngine`, preserving useful, final and primary energy as separate values. The EN15316-inspired modular chain is currently a pure calculator path only and is not yet wired into the default production calculation flow.
