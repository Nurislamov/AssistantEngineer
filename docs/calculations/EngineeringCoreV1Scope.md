...
|---|---|
| Transmission heat transfer | Steady-state component formula `Q = U * A * ?T * correctionFactor`. |
| Ventilation and infiltration | Sensible-only airflow heat transfer `Q = ? * cp * Vdot * ?T`. |
...
When added, validation should be treated as comparative engineering validation with documented tolerances, not as exact watt-by-watt equivalence.
...
- `No ExternalReferenceCovered claim without documented external fixture, tolerance and passing comparison test.`
