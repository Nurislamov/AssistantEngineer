...
internal sealed class Iso52016VerificationRunner
{
    private const string RegistryRelativePath = "docs/verification/Iso52016VerificationRegistry.json";
    private static readonly StringComparer PathComparer = OperatingSystem.IsWindows()
        ? StringComparer.OrdinalIgnoreCase
        : StringComparer.Ordinal;
    private readonly Iso52016Verifier verifier;

    public Iso52016VerificationRunner(string repoRoot, Iso52016VerificationRegistry registry)
    {
        verifier = new Iso52016Verifier(repoRoot, registry);
    }

    public void ListStages() => verifier.ListStages();

    public void VerifyStage(string stageId, bool skipTests) =>
        verifier.VerifyStage(stageId, skipTests);

    public void VerifyAll(bool skipTests, bool releaseReady, bool requireCleanGit) =>
        verifier.VerifyAll(skipTests, releaseReady, requireCleanGit);
