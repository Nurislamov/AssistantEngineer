...
$keys = @{
    ...
    TelegramBootstrapOwner = "TELEGRAM_BOOTSTRAP_OWNER_CHAT_ID"
...
$bootstrapOwnerConfigured =
    -not [string]::IsNullOrWhiteSpace($values[$keys.TelegramBootstrapOwner]) -or
    -not [string]::IsNullOrWhiteSpace($values[$keys.BootstrapOwner]) -or
    $allowedChatConfigured
...
$telegramAccessConfigured = $bootstrapOwnerConfigured -or $telegramAllowlistConfigured
...
if ($AllowPlaceholders) {
    ...
    if ($telegramEnabled) {
        ...
        if (-not $telegramAccessConfigured -and -not $discoveryEnabled) {
            throw "Telegram is enabled but bootstrap owner/access configuration is empty. Configure BootstrapOwnerChatId, AllowedChatIds, or AllowedUsernames."
        }
...
Write-Host "Telegram bootstrap owner configured: $bootstrapOwnerConfigured"
