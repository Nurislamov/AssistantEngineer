...
public class DomainBoundaryTests
{
    ...
    {
        Assert.True(AssistantEngineer.SharedKernel.ValueObjects.Area.FromSquareMeters(value).IsFailure);
        Assert.True(AssistantEngineer.SharedKernel.ValueObjects.Power.FromWatts(value).IsFailure);
        Assert.True(AssistantEngineer.SharedKernel.ValueObjects.Temperature.FromCelsius(value).IsFailure);
        Assert.True(AssistantEngineer.SharedKernel.ValueObjects.ThermalTransmittance.FromValue(value).IsFailure);
        Assert.True(AssistantEngineer.SharedKernel.ValueObjects.SolarHeatGainCoefficient.FromValue(value).IsFailure);
    }

    [Fact]
    public void GenericResultRejectsNullSuccessValue()
    {
        Assert.Throws<ArgumentNullException>(() => AssistantEngineer.SharedKernel.Primitives.Result<string>.Success(null!));
    }
    ...
    public async Task FullHeatRecoveryEliminatesVentilationHeatingLoss()
    {
        ...
        var room = floor.AddRoom(
            "Office",
            AssistantEngineer.SharedKernel.ValueObjects.Area.FromSquareMeters(20).Value,
            3,
            AssistantEngineer.SharedKernel.ValueObjects.Temperature.FromCelsius(22).Value,
            AssistantEngineer.SharedKernel.ValueObjects.Temperature.FromCelsius(-15).Value).Value;
